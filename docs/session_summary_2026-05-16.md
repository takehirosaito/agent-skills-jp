...
> 完了: 2026-05-16(昼前)
> 総コスト: **$100.84**(承認上限 $150 の 67%、Phase 2 全 retry + Chinese-leak QA まで含む)
...
| サイト掲載(検索可能) | 3,862 | **10,160** | **2.63 倍** |
| **ダウンロード可能(寛容ライセンス)** | **1,336** | **7,066** | **5.29 倍** |
| 日本語訳 description | 約 6,800 | **約 11,000(配信全件)** | +62% |
| 日本語訳 SKILL.md 本文 | (実装なし) | **10,076(配信中 99%)** | **新規対応** |
| 月額運用コスト | 約 950 円 | 約 2,400 円 | +1,450 円 |

**プレス v1「約 4,000 件」公約に対し、現在は収録 10,160 件 / DL 可 7,066 件 と、いずれも 2 倍以上の達成。**
...
- **副作用**: 配信 12,391 → **10,160 件**、DL 可 8,643 → **7,066 件**(質的整理で減少)

### 1.5 Phase 2 — SKILL.md 本文 → 日本語訳
- **コスト**: 累計 **$81.01**(初回 $47.47 + retry $20.10 + retry $13.44)
- **結果**: 8,681 / 8,777 件成功(98.9%)、残 96 件は context-length 等で失敗
- **複数 retry の理由**: 初回はクレジット枯渇、2 回目は月次利用上限到達 → ユーザがそれぞれ補充・引上げで段階的に再開
- **最終カバレッジ**:
  - 配信全体 10,160 件中 **10,076 件(99.2%)** が日本語本文持ち
  - 寛容ライセンス 7,066 件中 **7,002 件(99.1%)** が日本語本文持ち
- **モデル選択**: Sonnet 4.6 だと全件で $200〜700 となり $150 cap 超過するため、品質ほぼ同等の **Haiku 4.5** を採用

### 1.6 Phase 3 — プレス v2 原稿
- **コスト**: $0
- **成果物**: `docs/press_release_v2.md`
- **訴求**: 「3 日で収録 2.63 倍」「DL 可 5.29 倍」「本文も日本語化 10,076 件」
...
- **リード文**: 150 字、5/13 公開時の 3,862 件 → 10,160 件、寛容 7,066 件の本文 ZIP 配布対応を訴求
...
3. **(済)Anthropic API クレジット + 月次上限調整** → 完了済。本文翻訳カバー 99.2% 達成

### 優先度: 中

4. **(ほぼ完了)本文翻訳** → 残 96 件は context-length 制約で失敗中。長文 SKILL.md の分割翻訳を実装すれば 100% 到達
...
| Phase 2 retry 2 | Anthropic Haiku 4.5 | クレジット復旧後 retry +1,792 件 | $20.10 |
| Phase 2 retry 3 | Anthropic Haiku 4.5 | 月次上限引上げ後 retry +1,684 件 | $13.44 |
...
- 15,398 件の records(うち 10,160 件が EXCLUDE_HIDDEN フィルタを通過)
- 7,066 件の permissive(downloadable)
- 10,076 件 content_full_ja(日本語本文)持ち
...
3. **検索体感**: https://agent-skills.jp/search?q=react → 10,160 件のうちの上位寛容スキルが表示される
